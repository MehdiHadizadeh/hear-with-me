...
// Placeholders the HTML carries, filled per request. `%ORIGIN%` because a
// link preview needs an absolute URL that only the request knows, and
// `%TRANSPORT%` because the client can be driven either way and must not have
// to guess which server is behind it.
const placeholders = (origin) => ({ '%ORIGIN%': origin, '%TRANSPORT%': 'sse' });
  ...
  return fs.stat(filePath, (err, stat) => {
    ...
    if (ext === '.html') {
      return fs.readFile(filePath, 'utf8', (readErr, html) => {
        ...
        let filled = html;
        for (const [token, value] of Object.entries(placeholders(origin))) {
          filled = filled.split(token).join(value);
        }
        const body = Buffer.from(filled, 'utf8');
